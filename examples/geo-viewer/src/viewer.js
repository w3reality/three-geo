...
<<<<<<< HEAD
  constructor(env, threelet) {
    ...
    this.mapHelper = new MapHelper({
      ...
      onBuildTerrain: (ll) => {
        this.reloadPageWithLocation(ll);
      },
      onMapZoomEnd: () => {
  ...
  static _disposeObject(obj) {
    // cf. https://gist.github.com/j-devel/6d0323264b6a1e47e2ee38bc8647c726
  ...
  clearTerrainObjects() {
    ...
    //--
    this.sceneMeasure.children
      .filter((obj) => obj.name.startsWith("measure-mark-"))
      .forEach((mark) => {
  ...
  reloadPageWithLocation(ll, title = undefined) {
    ...
    if (0) {
      ...
      // update leaflet
      this.mapHelper.update(ll, this.tgeo.getProjection(ll, this._radius));
      this.plotCamInMap(this.camera);
...
>>>>>>> 54a2a0614b2fcac0c41e50749bae0b3415d39e79
  ...
  loadRgbDem(cb = this.nop) {
    if (this._isRgbDemLoaded) {
      return cb();
    }
    if (!Viewer.isTokenSet(this.env.tokenMapbox)) {
      return cb();
    }
  ...
  _addOrbit(orbit, segments = 128) {
    ...
    this._orbit = new THREE.LineLoop(
      geom,
  ...
  updateMeasure(mx, my) {
    ...
    if (isect !== null) {
      ...
      if (this.markPair.length === 1) {
        ...
        // when this.markPair.length === 0 or 2
  ...
  static toCoords(vec, nFloats = 3) {
    return `(${vec.x.toFixed(nFloats)}, ${vec.y.toFixed(
      nFloats
    )}, ${vec.z.toFixed(nFloats)})`;
  }
  ...
  showMsg(cam) {
    ...
    Viewer.appendText(
      this.msg,
      `pos [km]: ${Viewer.toCoords(Viewer.m2km(cam.position, unitsPerMeter))}`
    );
  ...
  showMeasureStats(_markPair) {
    ...
    if (_markPair.length === 1) {
      Viewer.appendText(
        this.msgMeasure,
        `points: ${Viewer.toCoords(
          Viewer.m2km(_markPair[0], unitsPerMeter)
        )} ->`
      );
      ...
      Viewer.appendText(
        this.msgMeasure,
        `points: ${Viewer.toCoords(p0km)} -> ${Viewer.toCoords(p1km)}`
      );
      Viewer.appendText(
        this.msgMeasure,
        `euclidean dist: ${p0km.distanceTo(p1km).toFixed(3)}`
      );
    }
  }
  showMsgTerrain() {
    ...
    Viewer.appendText(
      this.msgTerrain,
      `lat lng: (${ll[0].toFixed(4)}, ${ll[1].toFixed(4)})`
    );
    Viewer.appendText(
      this.msgTerrain,
      `satellite zoom resolution [11-17]: ${this._zoom}`
    );
  }
