...
class MapHelper {
  constructor(options = {}) {
    ...
    MapHelper.addSearchControl(this.map, (ll) => {
      // latlng
    ...
    //     foo: 'bar',
    if (_enableTiles) {
      // ?? when cache off, often getting 504 errors per CORS ........
  ...
  static mkBboxLayers(origin, ftBbox) {
    ...
    return [
      ...
      L.geoJson(line, this.mkOpts("white")).bindTooltip(
        `${dist.toFixed(1)} km`,
        {
          permanent: true,
          direction: "right",
  ...
  plotCam(cam) {
    ...
    let lineCam = MapHelper.mkLineCam(
      cam,
      this.origin,
      this.projection.unitsPerMeter,
      this.map.getZoom()
    );
    // console.log('lineCam:', lineCam);

    let camLatLngs = lineCam.geometry.coordinates.map((ll) =>
      MapHelper.swap(ll)
    );
  ...
  plotOrbit(orbit) {
    ...
    this.orbitMarker = L.marker(llTarget)
    ...
    this.orbitCircle = L.geoJson(circle, {
      style: (feature) => {
        return {
          dashArray: "3, 5",
          color: "#ff00ff",
  ...
  // update stuff --------
  clearBboxLayers() {
    this._bboxLayers.forEach((layer) => {
