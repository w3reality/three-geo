...
class RgbModel {
      ...
      Fetch.fetchTile(
        ...
        (tile) => {
          if (tile) {
            ...
            console.log(
              `fetchTile() failed for rgb dem of zp: ${zoompos} (count: ${count}/${zpEle.length})`
            );
          }
  ...
  addTile(tile, zoomposEle, zpCovered, bbox) {
    ...
    console.log(
      `now ${this.dataEleCovered.length} satellite tiles in dataEleCovered`
    );
  }
  ...
  static _build(
    ...
    dataEle.forEach(([zoompos, arr, zoomposEle]) => {
      ...
      let cSegments = this.resolveSeams(
        arr,
        this.getNeighborsInfo(dataEle, dataEleIds, zoompos)
      );
    ...
    Fetch.fetchTile(zoompos, apiSatellite, token, useNodePixels, (pixels) => {
      ...
      if (pixels) {
        ...
        tex = new THREE.DataTexture(
          this.createDataFlipY(pixels.data, pixels.shape),
          pixels.shape[0],
          pixels.shape[1],
          THREE.RGBAFormat
        );
