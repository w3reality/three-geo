...
class VectorModel {
      ...
      Fetch.fetchTile(
        ...
        (tile) => {
          if (tile) {
            ...
            console.log(
              `fetchTile() failed for vector dem of zp: ${zoompos} (count: ${count}/${zpEle.length})`
            );
          }
  ...
  addTile(tile, zoompos) {
    ...
    for (let i = 0; i < tile.layers.contour.length; i++) {
      const feat = tile.layers.contour
        .feature(i)
  ...
  _buildContours(polygon, radius) {
    const eleList = uniq(
      this.geojson.features.map((feat) => feat.properties.ele)
    ).sort((a, b) => a - b);
    ...
    return VectorModel._getContours(
      eleList,
      this.geojson,
      polygon,
      radius * radius * 2000000
    ); // maxArea: (r * sqrt2 * 1000)**2
  }
    ...
    for (let x = 0; x < eleList.length; x++) {
      ...
      try {
        ...
        if (0) {
          ...
          mergedElevationPoly = turfIntersect(
  ...
  _buildModelThree(contours, nw, se) {
    ...
    const colorRange = _getColorRange([0x231918, 0xed6356], contours.length);
  ...
  _buildSlice(coords, iContour, color, contours, nw, se) {
    ...
    const setVertices = (geom, vertices) =>
      geom.setAttribute(
        "position",
