...
class Utils {
    ...
    static betterSquare(center, radius) {
        ...
        for (let i = 0; i < 4; i++) {
         coordinates.push(turfDestination(center, cross, i * -360 / 4 + 45, {}).geometry.coordinates);
        }
    ...
    static polygonToBbox(coords, bufferRadius) {
        ...
        const xLength = turfDistance(turfHelpers.point(polyBbox.geometry.coordinates[0][0]),turfHelpers.point(polyBbox.geometry.coordinates[0][2])) //distance in km
        const yLength = turfDistance(turfHelpers.point(polyBbox.geometry.coordinates[0][3]),turfHelpers.point(polyBbox.geometry.coordinates[0][1])) //distance in km
        ...
        const squareBbox = turfBbox(turfBboxPolygon(turfBbox(turfCircle(centerPoint, radius, { steps: 64 }))))
